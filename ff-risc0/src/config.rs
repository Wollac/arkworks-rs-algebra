...
///
/// `N` is the limb count in `u64` units. Only `N = 4` (256-bit) and `N = 6` (384-bit) are
/// supported on the `zkvm` target, enforced by the `BigInt<N>: FieldFfi` bound.
    ...
    BigInt<N>: FieldFfi,
{
    ...
    /// An integer `b` such that there exists a multiplicative subgroup of size `b^k`.
    const SMALL_SUBGROUP_BASE: Option<u32> = None;

    /// The integer `k` such that there exists a multiplicative subgroup of size
    /// `Self::SMALL_SUBGROUP_BASE^k`.
